...
async function startServer() {
  ...
  app.post("/api/create-topup-order", async (req, res) => {
    try {
      ...
      // Update order with PayOS details
  ...
  app.post("/api/payos-webhook", async (req, res) => {
    try {
      ...
      // Verify webhook signature (using checkout sdk or manual verify)
      // For demo, we verify the signature if the CHECKSUM_KEY is provided
      // Use payos.verifyPaymentWebhookData(webhookData)
      ...
      if (verifiedData.code === '00' && verifiedData.desc === 'success') {
        ...
        // Atomically update wallet and set order to PAID
        await db.runTransaction(async (transaction) => {
          ...
          // Also update total top-up in user profile if exists
        ...
        console.log(`Successfully credited wallet for user ${userId}, Order: ${orderCode}`);
      }
